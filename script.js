...
function resp_overlay(event) {
    ...
    document.getElementById('resp_image').innerHTML = ``;
    for (i = 0; i < fotoTitles.length; i++) {
        document.getElementById('resp_image').innerHTML += `<li>${fotoTitles[i]}</li>`
        document.getElementById('resp_image').innerHTML += `<img src="./img/${fotoTitles[i]}.jpg" alt="">`
    }
}
